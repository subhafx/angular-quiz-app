...
export class QuizService {
  ...
  displayTimeElapsed() {
    ...
    const minutes = Math.floor((this.seconds / 60) % 60) < 10 ? '0' + Math.floor((this.seconds / 60) % 60) :
  ...
  getData() {
    return this.fireStore.collection('quizData').get();
  }
  ...
  getUserByEmail(email: string) {
    return this.fireStore.collection('userData').ref.where('email', '==', email).get();
  }
  ...
  sendResult(userId: string, resultData: any) {
    return this.fireStore.collection('userData').doc(userId).update({ resultData });
  }
